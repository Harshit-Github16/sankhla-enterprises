...
export default function SolarERPApp({
  ...
  const handleDownloadApp = async () => {
    if (!deferredPrompt) {
      addToast("App installation is not supported or already installed on this device.", "error");
      return;
    }
  ...
  if (!isHydrated) {
    return (
      <div className="flex h-screen bg-[#FAFBFC] overflow-hidden text-[#111827]">
        {/* Skeleton Sidebar */}
        <aside className="hidden md:flex w-64 border-r border-[#E5E7EB] bg-white flex-col justify-between shrink-0 p-4">
        ...
        <main className="flex-1 flex flex-col h-full overflow-hidden bg-[#FAFBFC]">
          {/* Skeleton Header */}
          <header className="bg-white border-b border-[#E5E7EB] px-6 py-4 flex items-center justify-between shrink-0">
          ...
          <div className="flex-1 overflow-y-auto p-6 space-y-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
              {[1, 2, 3, 4].map(i => (
                <div key={i} className="h-28 bg-white border border-gray-100 rounded-md p-4 flex flex-col justify-between">
            ...
            <div className="bg-white border border-gray-100 rounded-md p-6">
              ...
              <div className="space-y-3">
                <div className="h-10 bg-gray-50 rounded w-full border-b border-gray-100 animate-pulse"></div>
                {[1, 2, 3, 4, 5].map(i => (
                  <div key={i} className="h-12 bg-white border-b border-gray-50 rounded w-full flex items-center px-4 gap-4 animate-pulse">
      ...
      <aside className={`fixed inset-y-0 left-0 z-40 w-64 border-r border-[#E5E7EB] bg-white flex flex-col justify-between shrink-0 transform transition-transform duration-300 md:static md:translate-x-0 ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'
        ...
        <div className="p-4 border-t border-[#E5E7EB] space-y-2">
          <button
            ...
            className="w-full flex items-center justify-center gap-2 px-3 py-2 text-xs font-bold text-white bg-gradient-to-r from-blue-600 to-[#1E3A8A] hover:from-blue-700 hover:to-blue-900 rounded-md transition duration-150 shadow-sm"
          >
